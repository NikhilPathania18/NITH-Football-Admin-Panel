...
export default function UpdateTeamDetails() {
    ...
    const handleSubmit = async () => {
      try {
        ...
        const {data} = await API.updateTeamDetails(id,teamData);
        if(data.success){
          resetForm();
          setLogo(null)
          setSelectedPlayers([])  
    ...
    useEffect(()=>{
      const fetchData = async(id) => {
        try {
          ...
          const teamPlayers = data?.teamDetails.players;

          //setallplayers
          const res = await player.playerList()
          const temp = [];
          const temp2 = [];
          res.data.playersList.forEach((plr,index) => {
            temp.push({
              title: `${index+1}. ${plr?.name} (${plr?.rollNo})`,
              _id: plr._id
            })
            const doesExist = teamPlayers.some(obj => obj['_id'] === plr._id)
            if(doesExist){
              temp2.push({
                title: `${index+1}. ${plr?.name} (${plr?.rollNo})`,
                _id: plr._id

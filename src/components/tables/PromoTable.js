...
export default function PromoTable({ thead, tbody }) {
    const [alertModal, setAlertModal] = useState(false);
    const [data, setData] = useState([]);

    let newData = data
    const deletePromo = async(id,index) => {
    ...
    return (
        <Box className="mc-table-responsive">
            {data.length===0&&<EmptyList title={'Promos'} />}
            {
                data.length!==0&&
                        ...
                        <Tr key={ index }> 
                            ...
                            <Td>
                                <Box className="mc-table-trip md">
                                    ...
                                    <Box className="mc-table-group">
                                        <Heading as="h6">{ item?.promoCode }</Heading>
                                        {/* <Text>{ item?.en?. }</Text> */}
                                    </Box>
                                </Box>
                            </Td>
                            ...
                            <Td>
                                <Box className="mc-table-trip md">
                                    ...
                                    <Box className="mc-table-group">
                                        {/* <Heading as="h6">{ item?.are?.title }</Heading> */}
                                        <Text>{ item?.promoUsageLimit }</Text>
                                    </Box>
                                </Box>
                            </Td>
                            <Td>
                            ...
                            <Td>
                                <Box className="mc-table-trip md">
                                    ...
                                    <Box className="mc-table-group">
                                        {/* <Heading as="h6">{ item?.are?.title }</Heading> */}
                                        <Text>{ item?.promoExpiry }</Text>
                                    </Box>
                                </Box>
                            </Td>
                            <Td>
                                <Box className="mc-table-action">
                                    <Anchor href={`/promo-view/${item._id}`} title="View" className="material-icons view">visibility</Anchor>
                                    <Anchor href={`/edit-promo/${item._id}`} title="Edit" className="material-icons edit">edit</Anchor>
            ...
            <Modal show={ alertModal.active } onHide={()=> setAlertModal({active: false,id: '', index: ''})}>
                <Box className="mc-alert-modal">
                    ...
                    <Text as="p">Want to delete this Promo?</Text>
                    <Modal.Footer>
                        <Button type="button" className="btn btn-secondary" onClick={()=> setAlertModal({active: false,id: '', index: ''})}>nop, close</Button>
                        <Button type="button" className="btn btn-danger" onClick={()=> {deletePromo(alertModal.id,alertModal.index); setAlertModal({active: false,id: '', index: ''})}}>yes, delete</Button>
                    </Modal.Footer>
                </Box>
            </Modal>
        </Box>

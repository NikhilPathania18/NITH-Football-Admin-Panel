...
export default function TripsTable({ thead, tbody }) {
    ...
    const deleteBoat = async(id,index) => {
        const res = await API.boatYachtDeleteById(id);
    ...
    return (
        <Box className="mc-table-responsive">
            {data.length===0&&<EmptyList title={'Addons'} />}
            {
                data.length!==0&&
                        ...
                        <Tr key={ index }> 
                            ...
                            <Td>
                                <Box className="mc-table-action">
                                    <Anchor href={`/boat-view/${item._id}`} title="View" className="material-icons view">visibility</Anchor>
                                    <Anchor href={`/edit-boat/${item._id}`} title="Edit" className="material-icons edit">edit</Anchor>
                                    <Button title="Delete" className="material-icons delete" onClick={(e)=>{ setAlertModal({active:true, id:item?._id , index: index})}}>delete</Button>
                                </Box>
                            </Td>
                        </Tr>
                    ))}
            ...
            <Modal show={ alertModal.active } onHide={()=> setAlertModal({active: false,id: '', index: ''})}>
                <Box className="mc-alert-modal">
                    ...
                    <Modal.Footer>
                        ...
                        <Button type="button" className="btn btn-danger" onClick={()=> {deleteBoat(alertModal.id,alertModal.index); setAlertModal({active: false,id: '', index: ''})}}>yes, delete</Button>
                    </Modal.Footer>
                </Box>
            </Modal>
        </Box>
